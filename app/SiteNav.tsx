...
export function SiteNav() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 bg-white px-[18px] pb-3 pt-[max(8px,env(safe-area-inset-top,0px))] max-[420px]:px-3">
        <div className="mx-auto flex w-full min-w-0 max-w-[564px] min-h-16 items-center justify-between gap-[7px] rounded-[18px] border border-slate-200 bg-white px-[7px] py-1.5 shadow-[0_8px_18px_rgba(0,0,0,0.18)] max-[420px]:gap-1 max-[420px]:px-1.5">
          <Link
            href="/"
            className="flex min-w-0 flex-1 items-center gap-2 pr-1 transition-opacity hover:opacity-90 max-[420px]:gap-1.5 max-[420px]:pr-0.5"
            onClick={closeDrawer}
          >
            <Image
              src="/logo.jpeg"
              ...
              width={50}
              height={50}
              className="h-[50px] w-[50px] shrink-0 rounded-full border border-gray-200 object-cover max-[420px]:h-[42px] max-[420px]:w-[42px]"
              priority
            />
            <span className="min-w-0 flex-1 truncate text-[25px] font-extrabold leading-tight tracking-tight text-[#020617] max-[420px]:text-[19px]">
              NEPAL Motor
            </span>
          </Link>
          ...
          <div className="flex shrink-0 items-center gap-0.5">
            <a
              href={HELPLINE_TEL}
              className="flex items-center justify-center rounded-lg p-[7px] text-[#0f172a] transition-colors hover:bg-slate-100 max-[420px]:p-1.5"
              aria-label="Call NEPAL Motor"
            >
              <IconCallOutline className="h-6 w-6 max-[420px]:h-[22px] max-[420px]:w-[22px]" />
            </a>
            <button
              type="button"
              className="flex items-center justify-center rounded-lg p-[7px] text-[#0f172a] transition-colors hover:bg-slate-100 max-[420px]:p-1.5"
            ...
            >
              <IconMenu className="h-[26px] w-[26px] max-[420px]:h-[22px] max-[420px]:w-[22px]" />
            </button>
          </div>
        </div>
      </header>
      ...
      {portalReady && drawerOpen
        ? createPortal(
            <div
              className="fixed inset-0 z-[200] flex min-h-[100dvh] flex-row items-start justify-start overflow-x-hidden overflow-y-auto pl-4 pt-0 md:min-h-0 md:justify-center md:pl-0 md:pt-2"
              role="dialog"
              aria-modal="true"
            ...
            >
              <button
                ...
                onClick={closeDrawer}
              />
              <div className="relative z-[201] min-w-0 w-full max-w-[564px] md:flex md:justify-end md:px-[18px]">
                <div
                  className="relative z-[201] mt-[max(32px,calc(env(safe-area-inset-top,0px)+8px))] flex h-[70vh] w-[min(340px,calc(100vw-2rem),82vw)] max-w-[340px] flex-col overflow-hidden rounded-[20px] bg-[#f1f5f9] shadow-[0_4px_24px_rgba(0,0,0,0.28)] max-[420px]:mt-[max(28px,calc(env(safe-area-inset-top,0px)+6px))] max-[420px]:h-auto max-[420px]:max-h-[min(78dvh,calc(100dvh-4.5rem))] md:mt-6 md:h-auto md:max-h-[min(720px,85vh)] md:w-[340px]"
                >
                    ...
                    <div className="mx-3.5 my-0.5 h-px shrink-0 bg-slate-200" />

                    <div className="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain md:min-h-0">
                      ...
                      <div className="mx-3.5 my-1 h-px shrink-0 bg-slate-300" />
